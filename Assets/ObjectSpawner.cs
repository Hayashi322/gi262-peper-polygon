...
public class ObjectSpawner : MonoBehaviour
{
    ...
    // ตัวแปรสำหรับตำแหน่ง checkpoint
    public Vector3 checkpointPosition;  // ตัวแปรสำหรับตำแหน่ง checkpoint
                                        // ฟังก์ชันการ spawn วัตถุ (ทำการ spawn หลังจาก 2 วินาที)
    void Start()
    {
        // เรียก Coroutine เพื่อให้มันทำงานหลังจาก 2 วินาที
        StartCoroutine(WaitForTimeAndSpawn());
    }
    // Coroutine สำหรับการรอ 2 วินาที
    ...
    void SpawnObject()
    {
        ...
        {
            // ตรวจสอบว่าเป็นตำแหน่งที่เป็น FLOOR และไม่ใช่ WALL หรือ checkpoint
            if (floorTileMap.HasTile(pos) && floorTileMap.GetTile(pos) == floorTile && !IsWall(pos) && !IsCheckpoint(pos))
            {
    ...
    // ฟังก์ชันตรวจสอบว่าเป็น checkpoint หรือไม่
    bool IsCheckpoint(Vector3Int position)
    {
        // ตรวจสอบว่าเป็นตำแหน่งที่เหมือนกับตำแหน่ง checkpoint หรือไม่
        return checkpointPosition != Vector3.zero && new Vector3(position.x, position.y, 0) == checkpointPosition;
    }
}
